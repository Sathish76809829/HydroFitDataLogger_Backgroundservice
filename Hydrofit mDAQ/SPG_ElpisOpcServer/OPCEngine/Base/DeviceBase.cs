...
namespace Elpis.Windows.OPC.Server
{
    ...
    public class DeviceBase : SerializableDependencyObject, INotifyPropertyChanged
    {
        ...
        private AllowedSamplingRates samplingRate;//= 100;
        ...
        public AllowedSamplingRates SamplingRate
        {
            get
            {
                return samplingRate;
        ...
        private int pump_test_rpm;//= 100;
        [Description("Please enter the pulse rate Between 1-120."), DisplayName("Pump_test_Rpm(PPr)*"), PropertyOrder(10)]
        public int Pump_Test_Rpm
        {
            get
            {
                return pump_test_rpm;
            }
            set
            {
                ...
                {
                    pump_test_rpm = value;
                    OnPropertyChanged("Pump_Test_Rpm");
                }
                else
                {
                    // You can throw an exception, log an error, or take any other appropriate action here.
                    throw new ArgumentOutOfRangeException("Pump_Test_Rpm", "RPM value must be between 1 and 120.");
                }
            }
        }
        private int break_test_rpm;//= 100;
        [Description("Please enter the pulse rate Between 1-120."), DisplayName("Break_test_Rpm(PPr)*"), PropertyOrder(11)]
       
        public int Break_Test_Rpm
        {
            get
            {
                return break_test_rpm;
            }
            set
            {
                // Check if the value is within the valid range
                if (value >= 1 && value <= 120)
                {
                    break_test_rpm = value;
                    OnPropertyChanged("Break_Test_Rpm");
                }
                else
                {
                    // You can throw an exception, log an error, or take any other appropriate action here.
                    throw new ArgumentOutOfRangeException("Break_Test_Rpm", "RPM value must be between 1 and 120.");
                }
        ...
        //[Description("Please enter the sampling rate in milliseconds."), DisplayName("SamplingRate(ms)*"), DefaultValue((int)100), PropertyOrder(9)]
        ...
        //private AllowedSamplingRates samplingRate = (AllowedSamplingRates)100;
        //[Description("Please enter the sampling rate in milliseconds."), DisplayName("SamplingRate(ms)*"), DefaultValue((int)100), PropertyOrder(9)]
        //public AllowedSamplingRates SamplingRate
        //{
        //    get { return samplingRate; }
