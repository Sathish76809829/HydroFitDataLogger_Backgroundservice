...
namespace ElpisOpcServer.SunPowerGen
{
    public class Helper
    {
        ...
        public static int sufficscount = 0;
        
        //public static object channelConfig;
        ...
        public static string StringformatConfigData(List<Tuple<string, Elpis.Windows.OPC.Server.Tag>> selectedItems)
        {
            bool IsEnable = true;
            ...
            var result = new
            {
                samplingRate = tcpdevice.SamplingRate,
                PumpAPPR = tcpdevice.Pump_Test_Rpm,
                BreakAPPR = tcpdevice.Break_Test_Rpm,
                channelConfig = selectedItems.Where(s => s.Item2.Address != "start-stop").Select(s => $"{s.Item2.TagId};{s.Item2.Address};{IsEnable};{Indexcount++};{s.Item2.DataType}")
            };
        ...
        public static List<parsedDeviceData> ParsedDeviceDatas(string JsonData)
        {
            try
            {
                var datadevices = JsonConvert.DeserializeObject<DeviceDataModel[]>(JsonData);

                if (datadevices != null && datadevices.Length > 0)
                {
                    string[] array = datadevices[0].device.Split('>');
                    List<parsedDeviceData> parsedDeviceDatas = new List<parsedDeviceData>();
                    for (int counter = 0; counter < datadevices[0].SignalDataList.Length; counter++)
                    {
                        var innerArray = datadevices[0].SignalDataList[counter].Split('>');
                        parsedDeviceData parsedDevice = new parsedDeviceData
                        {
                           //deviceId = array[0],
                            SignalId = innerArray[0],
                            DataValue = Convert.ToDouble(innerArray[1])//.Split('@')[0],
                                                       //value = item.Split('>')[1].Split('@')[0],
                                                       //timeStamp = Convert.ToDateTime(item.Split('@')[1]),
                            //timeStamp = array[1].Split('<')[0]
                        };
                ...
                //var datadevice = datadevices.FirstOrDefault();
                //if (datadevice != null)
                //{
                //    string[] strings = datadevice.device.Split('>');
                //    string deviceId = datadevice.device.Split('>')[0];
                //    DateTime time = Convert.ToDateTime(strings[1].Split('<')[0]);
                //    List<parsedDeviceData> parsedDeviceDatas = new List<parsedDeviceData>();
                ...
                //    return parsedDeviceDatas;
                //}
                #endregion using Foreach loop
                return null;
            }
            catch (Exception ex)
            {
                _logger.LogError("DataParser() : " + ex.Message);
                MessageBox.Show(ex.Message);
                return null;
            }
        }
        ...
        public static bool ConnectingDevice(bool isConnected, ObservableCollection<Elpis.Windows.OPC.Server.Tag> tagsCollection)
        {
            if (SunPowerGenMainPage.DeviceObject != null)
            {
                ...
                else if (deviceType == DeviceType.TcpSocketDevice)
                {
                    ...
                    // SunPowerGenMainPage.DeviceTcpClient = Helper.CreateTcpClient(((TcpSocketDevice)SunPowerGenMainPage.DeviceObject).IPAddress, ((TcpSocketDevice)SunPowerGenMainPage.DeviceObject).Port);
        ...
        public static string logpath = @"E:\Sathish.b\WorkingBackup\Hydrofit Datalogger_ToSatish\Hydrofit Datalogger\HydrofitDataLogger\Hydrofit mDAQ\SPG_ElpisOpcServer\ElpisOpcServer\bin\Debug";
        //public static string logfileName = "HydfitDatalog.txt";
        #region writeLogs 
        public static bool WriteLog(string strFileName, string strMessage)
        {
            try
            {
                FileStream objFilestream = new FileStream(string.Format("{0}\\{1}", logpath, strFileName), FileMode.Append, FileAccess.Write);
                StreamWriter objStreamWriter = new StreamWriter((Stream)objFilestream);

                // StreamWriter objStreamWriter = new StreamWriter(logfileName, append: true);
        ...
        //        {
        //            // Clear the file data before rewriting
        //            File.WriteAllText(strFileName, string.Empty);
        //        }

        //        // Open the file in append mode and write the log message
        //        using (StreamWriter objStreamWriter = new StreamWriter(strFileName, append: true))
        //        {
        //            objStreamWriter.WriteLine(strMessage);
        //        }
        ...
        //    catch (Exception ex)
        //    {
        //        // Handle any exceptions here (e.g., logging the exception)
        //        return false;
        //    }
        //}
        #endregion NewLogic

        #region Convert and Add to the file Adc to Physical values
        public static void AdcAndPhysicalQuantity(string fileContent)
        {
            try
            {
                string projectDirectory = Directory.GetCurrentDirectory();
                string filePath = Path.Combine(projectDirectory, "myFile.txt");

               // string additionalContent = "This is additional content that will be appended.";
                File.AppendAllText(filePath, Environment.NewLine + fileContent);
            }
            catch (Exception ex)
            {
        ...
        #endregion Convert and Add to the file Adc to Physical values
       
        public static double Calculate2AmsValue(float amsSensorMin, float amsSensorMax, float aMinConstant, float amsMaxConstant, double inputAdcValue)
        {
            //float m = (TwoAmsmaxConstant - TwoAmsminConstant) / (TwoAmssensorMax - TwoAmssensorMin);
            //double X = (1 / m) * (inputAdcValue - TwoAmsminConstant) + TwoAmssensorMin;

            float m1 = (AdcComputationConstant.MaxConstant_Current - AdcComputationConstant.MinConstant_Current) / (AdcComputationConstant.SensorMax_Current - AdcComputationConstant.SensorMin_Current);
            double X1 = (1 / m1) * (inputAdcValue - Convert.ToDouble(AdcComputationConstant.MinConstant_Current)) + Convert.ToDouble(AdcComputationConstant.SensorMin_Current);
            return X1;
        }
        public static double CalVolumetricEfficency(double flowInput, double Flowconstant)
        {
            double calVolumetricEfficency = (flowInput / Flowconstant) * 100;
            return calVolumetricEfficency;
        }
    }
